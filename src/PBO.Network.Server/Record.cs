        ...
        private static void AddInitingGameImplement(int room, InitingGame ig)
        {
            ...
            lock (igLocker)
            {
                ...
                if (pms != null)
                    ...
                    {
                        Append(Ig, new object[] { room, ig.Id, ig.Settings.Mode, "00", pm.Form.Species.Number, pm.Form.Index, pm.Nature, pm.AbilityIndex, pm.Ev.Hp, pm.Ev.Atk, pm.Ev.SpAtk, pm.Ev.Def, pm.Ev.SpDef, pm.Ev.Speed });
                ...
                if (ig.Settings.Mode.PlayersPerTeam() == 2)
                {
                    ...
                    if (pms != null)
                        foreach (var pm in pms)
                        {
                            Append(Ig, new object[] { room, ig.Id, ig.Settings.Mode, "01", pm.Form.Species.Number, pm.Form.Index, pm.Nature, pm.AbilityIndex, pm.Ev.Hp, pm.Ev.Atk, pm.Ev.SpAtk, pm.Ev.Def, pm.Ev.SpDef, pm.Ev.Speed });
                            foreach (var m in pm.Moves) Append(Ig, m.Move.Id);
                            Ig.WriteLine();
                        }
                    ...
                    if (pms != null)
                        foreach (var pm in pms)
                        {
                            Append(Ig, new object[] { room, ig.Id, ig.Settings.Mode, "11", pm.Form.Species.Number, pm.Form.Index, pm.Nature, pm.AbilityIndex, pm.Ev.Hp, pm.Ev.Atk, pm.Ev.SpAtk, pm.Ev.Def, pm.Ev.SpDef, pm.Ev.Speed });
                            foreach (var m in pm.Moves) Append(Ig, m.Move.Id);
                            Ig.WriteLine();
                        }
                }
                ...
                if (pms != null)
                    foreach (var pm in pms)
                    {
                        Append(Ig, new object[] { room, ig.Id, ig.Settings.Mode, "10", pm.Form.Species.Number, pm.Form.Index, pm.Nature, pm.AbilityIndex, pm.Ev.Hp, pm.Ev.Atk, pm.Ev.SpAtk, pm.Ev.Def, pm.Ev.SpDef, pm.Ev.Speed });
                        foreach (var m in pm.Moves) Append(Ig, m.Move.Id);
                        Ig.WriteLine();
                    }
                Ig.Flush();
