...
namespace PokemonBattleOnline.Test
{
    class TestClient
    {
        ...
        public TestClient(string name, Seat seat)
        {
            Name = name;
            C = new PboClient("ws://localhost:9999");
            Seat = seat;
            C.Init(Name, "4p", seat);
            Thread.Sleep(500);
        }
            ...
            switch (line)
            {
                case "ok":
                    ...
                    if (Seat == Seat.Player00)
                        C.RoomController.PropertyChanged += (sender, e) =>
                        {
                            if (e.PropertyName == "Game")
                            {
                                C.Game.LogAppended += (t, s) =>
                                {
                    ...
                    C.RoomController.PropertyChanged += (sender, e) =>
                    {
                        if (e.PropertyName == "PlayerController" && C.PlayerController != null)
                        {
                            PM = C.PlayerController.Player.Pokemons.ToArray();
                            C.PlayerController.RequireInput += (ir) => IR = ir;
                        }
                    };
                    C.RoomController.GamePrepare(new GameSettings(GameMode.Multi), Team.ToArray());
                    return;
                case "preview":
                    ShowTeam();
                    break;
