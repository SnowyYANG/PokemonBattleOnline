...
namespace LightStudio.PokemonBattle.Data
{
    ...
    public static class PokemonNatureHelper
    {
        ...
        public static StatType StatBoosted(this PokemonNature nature)
        {
            for (int i = 0; i < REVISES.GetLength(1); i++)
            {
                if (REVISES[(int)nature, i] == 11)
                    return (StatType)i + 1;
            }
            return StatType.Invalid;
        }

        public static StatType StatHindered(this PokemonNature nature)
        {
            for (int i = 0; i < REVISES.GetLength(1); i++)
            {
                if (REVISES[(int)nature, i] == 9)
                    return (StatType)i + 1;
            }
            return StatType.Invalid;
        }
