...
namespace PokemonBattleOnline.Game
{
    public class Team0791 : ITeamIO
    {
        ...
        private static PokemonData ReadData(BinaryReader reader)
        {
            ...
            if (identity > 0 && identity < 504)
            {
                ...
                if (itemId > 0) pm.Item = RomData.GetItem(itemId);
    ...
    public class TeamPO : ITeamIO
    {
        ...
        private static PokemonData LoadXmlData(XmlNode pm)
        {
            ...
            if (itemId > 0) pd.Item = RomData.GetItem(itemId);
            if (abilityId > 0) pd.Ability = abilityId;
            ...
            for (int i = 0; i < 4 && i < moves.Length; i++)
            {
                if (moves[i] > 0)
                {
                    var move = RomData.GetMove(moves[i]);
                    pd.AddMove(move);
                }
            }
        ...
        public string ExportString(PokemonBT pds)
        {
          ...
          //var ret = new StringBuilder();
            ...
            //    if (pm.Name != GameString.Current.Pokemon(pm.Form.Species.Number))
            //    {
            //        ret.AppendFormat(" ({0})", GameString.Current.Pokemon(pm.Form.Species.Number));
            //    }
