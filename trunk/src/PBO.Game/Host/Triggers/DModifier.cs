...
namespace PokemonBattleOnline.Game.Host.Triggers
{
  internal static class DModifier
  {
    public static Modifier Execute(DefContext def)
    {
      ...
      var n = der.Pokemon.Form.Type.Number;
      ...
      if (cat == MoveCategory.Special && der.Controller.Weather == Weather.IntenseSunlight)
        foreach (PokemonProxy pm in der.Controller.GetOnboardPokemons(der.Pokemon.TeamId))
          if (pm.Pokemon.Form.Type.Number == 421 && pm.Ability == As.FLOWER_GIFT) m *= 0x1800;
      switch (der.Item)
      {
        case Is.EVIOLITE:
          if (Data.GameDataService.GetEvolutions(der.Pokemon.Form.Type.Number).Any()) m = 0x1800;
          break;
        case Is.SOUL_DEW:
